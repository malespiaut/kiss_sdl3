...
int
kiss_maxlength(kiss_font font, int width, char* str1, char* str2)
{
...
int
kiss_textwidth(kiss_font font, char* str1, char* str2)
{
  char buf[KISS_MAX_LENGTH] = {0};
  int width = 0;
...
int
kiss_renderimage(SDL_Renderer* renderer, kiss_image image, int x, int y, SDL_Rect* clip)
{
  SDL_Rect dst = {0};
...
int
kiss_rendertext(SDL_Renderer* renderer, char* text, int x, int y, kiss_font font, SDL_Color color)
{
...
int
kiss_fillrect(SDL_Renderer* renderer, SDL_Rect* rect, SDL_Color color)
{
  if (!renderer || !rect)
  {
    return -1;
...
int
kiss_decorate(SDL_Renderer* renderer, SDL_Rect* rect, SDL_Color color, int edge)
{
...
int
kiss_image_new(kiss_image* image, char* fname, kiss_array* a, SDL_Renderer* renderer)
{
  char buf[KISS_MAX_LENGTH] = {0};
...
int
kiss_font_new(kiss_font* font, char* fname, kiss_array* a, int size)
{
  char buf[KISS_MAX_LENGTH] = {0};
...
SDL_Renderer*
kiss_init(char* title, kiss_array* a, int w, int h)
{
  ...
  r += kiss_font_new(&kiss_textfont, "font.ttf", a, kiss_textfont_size);
  r += kiss_font_new(&kiss_buttonfont, "font.ttf", a, kiss_buttonfont_size);
  ...
  r += kiss_image_new(&kiss_unselected, "unselected.bmp", a, renderer);
  if (r)
  {
    kiss_clean(a);
    return NULL;
